class PredictiveSearch extends HTMLElement {
  ...
  getSearchResults(searchTerm) {
    ...
    fetch(
      `${routes.predictive_search_url}?q=${encodeURIComponent(searchTerm)}&${encodeURIComponent('resources[type]')}=product&${encodeURIComponent('resources[limit]')}=10&${encodeURIComponent('resources[fields]')}=title,product_type,variants.title,tag&section_id=predictive-search`
    )
      .then(response => {
        if (!response.ok) {
